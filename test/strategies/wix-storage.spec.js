...
describe('wix-storage-strategy', () => {
  ...
  it('should set item with expiration in days', async () => {
    ...
    await capsule.setItem('shahata', 123, {namespace: 'wix', expiration: 60 * 60 * 48});
  });
  ...
  it('should set item with error handling', async () => {
    ...
    await expect(capsule.setItem('shahata', 123, {namespace: 'wix'})).to.be.rejectedWith(SERVER_ERROR);
  });

  it('should delete item', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).post('/_api/wix-user-preferences-webapp/delete',
  ...
  it('should delete item with scope', async () => {
    ...
    await capsule.removeItem('shahata', {namespace: 'wix', scope: {siteId: 456}});
  });
  ...
  it('should delete item with error handling', async () => {
    ...
    await expect(capsule.removeItem('shahata', {namespace: 'wix'})).to.be.rejectedWith(SERVER_ERROR);
  });

  it('should get item', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefForKey/wix/shahata')
  ...
  it('should get item with scope', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefForSite/wix/456/shahata')
      .reply(200, {shahata: 123});
    expect(await capsule.getItem('shahata', {namespace: 'wix', scope: {siteId: 456}})).to.equal(123);
  });

  it('should get item with error handling', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefForKey/wix/shahata').reply(500);
    await expect(capsule.getItem('shahata', {namespace: 'wix'})).to.be.rejectedWith(SERVER_ERROR);
  });

  it('should get item with 404 handling', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefForKey/wix/shahata').reply(404);
    await expect(capsule.getItem('shahata', {namespace: 'wix'})).to.be.rejectedWith(NOT_FOUND);
  });

  it('should get items', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefs/wix')
      .reply(200, {shahata: 123});
  ...
  it('should get items with scope', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefsForSite/wix/456')
      .reply(200, {shahata: 123});
    expect(await capsule.getAllItems({namespace: 'wix', scope: {siteId: 456}})).to.eql({shahata: 123});
  });

  it('should get items with error handling', async () => {
    const capsule = new DataCapsule({strategy: new WixStorageStrategy()});
    nock(BASE_URL).get('/_api/wix-user-preferences-webapp/getVolatilePrefs/wix').reply(500);
    await expect(capsule.getAllItems({namespace: 'wix'})).to.be.rejectedWith(SERVER_ERROR);
  });
